...
type Screen = 'home' | 'workout' | 'progress';
...
export default function App() {
  ...
  return (
    <>
      {screen === 'home' && (
        <Home history={history} onStart={startWorkout} onSignOut={signOut} />
      )}
      {screen === 'workout' && activeWorkout && (
        <WorkoutScreen
          ...
          onBack={() => {
            if (
              confirm(
                'Leave this session? Anything you logged but didn\'t save will be lost.'
              )
            ) {
              setActiveWorkout(null);
              setScreen('home');
